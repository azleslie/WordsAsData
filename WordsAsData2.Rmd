...
In written language, we use strings of characters to represent ideas, but we rely on other patterns that emerge through the aggregation of words to derive further meaning: their order, repetition, and immediate context, the selection of some rather than others, their wider associations. Similarly, quantitative text analysis is primarily concerned with characters, but we'll rely on the other data types in order to represent qualities like order, repetition, context, and so on.
...
Quantifying broad patterns of word use can reveal a lot about a text right away. We might begin to consider how the text represents gender, for example, by looking at its use of pronouns. `which` elements in our word_vector vector are the character string "her"? `which` searches for an exact character pattern within a vector. `==` signifies "is equal to."
```{r}
length(which(word_vector=="she"))
length(which(word_vector=="he"))
```

English verbs aren't differentiated quite as reliably by suffix as verbs in other languages, but we can at least get a sense based on usage of basic "to be" conjugations. We can signify "or" with the "or" operator, `|`.
```{r}
...
*The Grandissimes* is a historical romance with male protagonists, so this makes sense. It would come in handy, however, if we were trying to compare the novel to others or distinguish novels in this genre from those in others. In fact, a lot of predictive modeling in digital humanities boils down to the relative frequencies of common yet vital words like these.

## Structuring Data

Vectors are useful, but by themselves they are somewhat information-poor: they can only store a sequence of individual observations. We often use two-dimensional structures like spreadsheets (like Excel documents), arrays, and other types of tabular formats in order to organize data in a way that is easily intuited for human users and easily processed by computers. In R this format is called a data frame. We can turn a vector into a one-column data frame called "noveldf" with the function `data.frame`, specifying "word" as the column name.
```{r}
noveldf <- data.frame(word=word_vector)
```

We can use the `$` operator to refer to just one column of a data frame at a time. Type "noveldf$word" into the Console and click enter. Taken by itself, a single column still functions as a vector.
...
We organize words into groups at several concentric scales - sentences, paragraphs, and chapters - in order to better delineate the ideas they express and help readers better understand. Computers rely on groupings in order to make sense of patterns as well. We split our book into individual words as the smallest atomic unit of meaning (for linguistics or poetics, the atomic unit may be the syllable instead). When treating words as data, we want to reintroduce these groupings wherever possible as new variables.
...
First, let's rename some variable (column) names with the `rename` function. Note that `<-` is overwriting all_records with the new variable names: without `<-` R would simply print it differently in the console without actually changing all_records.
```{r}
noveldf <- noveldf %>%
  rename("Position" = "id") %>%
  rename("Word" = "word") %>%
...
Unsurprisingly, the most commonly used words in most texts are, well, a bit boring. In English, in fact, these will almost always be the most common words; this phenomenon is called Zipf's Law. Repetition of this most basic kind in language is important, and deviations between texts can sometimes tell us a lot about style. But we also want to know about distinctiveness.
...
Despite Bras Coupé's decisive impact on *The Grandissimes*, he mostly appears just in one segment of it, aside from some important foreshadowing and build-up. This graph helps us describe this peculiar experience of reading the novel.
...
What's striking here is that while "he" appears much more frequently in *The Grandissimes* overall, there are numerous chapters in which "she" is much more common. Reusing our code from the start of today, we can find the average ratio of first person male pronouns to first person female pronouns:
```{r}
...
As it turns out, very few chapters in *The Grandissimes* have pronoun ratios that come close to the overall ratio. As these calculations suggest, one could produce very different interpretations of gender in the novel simply based on the chapters from which they sourced examples.
